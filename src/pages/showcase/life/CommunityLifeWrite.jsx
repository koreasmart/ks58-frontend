...
const CommunityLifeWrite = () => {
  ...
  /**
   * imagePreviews : 화면에 보여줄 URL 배열 (blob URL 또는 서버 URL)
   * imageFiles    : imagePreviews 와 1:1 대응하는 배열
   *   - null  → 이미 서버에 있는 기존 이미지 (수정 모드에서 불러온 것)
   *   - File  → 사용자가 새로 선택한 파일 (아직 업로드 안 됨)
   * deletedServerUrls : 기존 이미지 중 사용자가 삭제한 서버 URL 목록
   */
  const [imagePreviews, setImagePreviews] = useState([]);
  const [imageFiles, setImageFiles] = useState([]);
  const [deletedServerUrls, setDeletedServerUrls] = useState([]);
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => console.error("지역 목록 조회 실패:", err));
  }, []);
  ...
  useEffect(() => {
    ...
    api
      .get("/community/life/schedules", { params: { mbrId: user.mbrId } })
      .then((res) => setSchedules(res.data))
      .catch((err) => console.error("내 AI 일정 목록 조회 실패:", err));
  }, [user]);
    ...
    const fetchEditPost = async () => {
      try {
        ...
        if (item.images?.length > 0) {
          serverUrls = item.images.map(getImageUrl);
        } else if (item.commMainImgUrl) {
          serverUrls = [getImageUrl(item.commMainImgUrl)];
        }

        setImagePreviews(serverUrls);
        setImageFiles(serverUrls.map(() => null));
  ...
  const handleRemoveImage = (removeIndex) => {
    const removedPreview = imagePreviews[removeIndex];
    const removedFile = imageFiles[removeIndex];

    if (removedFile === null) {
      setDeletedServerUrls((prev) => [...prev, removedPreview]);
    } else {
      URL.revokeObjectURL(removedPreview);
    }

    setImagePreviews((prev) => prev.filter((_, i) => i !== removeIndex));
    setImageFiles((prev) => prev.filter((_, i) => i !== removeIndex));
  };
  ...
  const normalizeTag = (tag) =>
    tag.trim().replace(/^#/, "").replace(/\s+/g, "");

  const finalTags = () =>
    Array.from(
      new Set(
        [...tags, normalizeTag(tagInput)]
          .map(normalizeTag)
          .filter(Boolean)
      )
    );
  ...
  // ─── 제출: 유효성 검사 → FormData로 파일 + 게시글 데이터 한 번에 전송 ───
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingImageUrls = imagePreviews
        .filter((_, i) => imageFiles[i] === null)
        .map((url) => url.replace(API_URL, ""));

      // 삭제할 기존 이미지 URL
      const deletedImageUrls = deletedServerUrls
        .map((url) => url.replace(API_URL, ""));
      ...
      if (isEditMode) {
        ...
        await api.post("/community1", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        alert("글이 등록되었습니다!");
      ...
      console.error("인생거리 등록/수정 실패:", err);
